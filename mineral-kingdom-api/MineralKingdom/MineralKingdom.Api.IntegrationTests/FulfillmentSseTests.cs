...
public sealed class FulfillmentSseTests : IClassFixture<PostgresContainerFixture>
{
  private readonly PostgresContainerFixture _pg;
  public FulfillmentSseTests(PostgresContainerFixture pg) => _pg = pg;

  [Fact]
  public async Task Fulfillment_sse_requires_auth()
  {
    await using var factory = new TestAppFactory(_pg.Host, _pg.Port, _pg.Database, _pg.Username, _pg.Password);
    using var client = factory.CreateClient();

    using var req = new HttpRequestMessage(HttpMethod.Get, $"/api/fulfillment-groups/{Guid.NewGuid()}/events");
  ...
  [Fact]
  public async Task Fulfillment_sse_forbids_non_owner()
  {
    await using var factory = new TestAppFactory(_pg.Host, _pg.Port, _pg.Database, _pg.Username, _pg.Password);
    ...
    using var req = new HttpRequestMessage(HttpMethod.Get, $"/api/fulfillment-groups/{groupId}/events");
    var res = await client.SendAsync(req, HttpCompletionOption.ResponseHeadersRead);
  ...
  [Fact]
  public async Task Fulfillment_sse_emits_initial_snapshot_for_owner()
  {
    await using var factory = new TestAppFactory(_pg.Host, _pg.Port, _pg.Database, _pg.Username, _pg.Password);
    ...
    all.Should().Contain(groupId.ToString());
  }
}