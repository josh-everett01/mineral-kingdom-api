...
public sealed class OpenBoxService
{
  ...
  public OpenBoxService(MineralKingdomDbContext db)
  {
    _db = db;
  }
    ...
    CancellationToken ct)
  {
    ...
    var box = new FulfillmentGroup
    {
      ...
      ShipmentRequestStatus = ShipmentRequestStatuses.None,
      ShipmentRequestedAt = null,
      ShipmentReviewedAt = null,
      ShipmentReviewedByUserId = null,
      CreatedAt = now,
      UpdatedAt = now
    };
    ...
    CancellationToken ct)
  {
    ...
    if (order.FulfillmentGroupId is Guid existingGroupId && existingGroupId != box.Id)
    {
      ...
      if (string.Equals(existingGroup.BoxStatus, "CLOSED", StringComparison.OrdinalIgnoreCase) ||
          string.Equals(existingGroup.BoxStatus, "LOCKED_FOR_REVIEW", StringComparison.OrdinalIgnoreCase))
    ...
    CancellationToken ct)
  {
    ...
    box.BoxStatus = "LOCKED_FOR_REVIEW";
    box.ClosedAt = now;
    box.UpdatedAt = now;

    box.ShipmentRequestStatus = ShipmentRequestStatuses.Requested;
    box.ShipmentRequestedAt = now;
